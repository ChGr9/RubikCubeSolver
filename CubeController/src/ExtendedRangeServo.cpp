...
//
ExtentedRangeServo::ExtentedRangeServo()
...
ESP32PWM * ExtentedRangeServo::getPwm(){

	return &pwm;
}

int ExtentedRangeServo::attach(int pin)
{

    return (this->attach(pin, DEFAULT_uS_LOW, DEFAULT_uS_HIGH));
}

int ExtentedRangeServo::attach(int pin, int min, int max)
{
...
void ExtentedRangeServo::detach()
{
    if (this->attached())
    {
...
void ExtentedRangeServo::write(int value)
{
...
void ExtentedRangeServo::writeMicroseconds(int value)
{
...
void ExtentedRangeServo::release()
{
    if (this->attached())   // ensure channel is valid
        pwm.write(0);
}

int ExtentedRangeServo::read() // return the value as degrees
{
    return (map(readMicroseconds()+1, this->min, this->max, 0, 180));
}

int ExtentedRangeServo::readMicroseconds()
{
    int pulsewidthUsec;
    if (this->attached())
    { 
...
bool ExtentedRangeServo::attached()
{
    return (pwm.attached());
}

void ExtentedRangeServo::setTimerWidth(int value)
{
...
int ExtentedRangeServo::readTimerWidth()
{
    return (this->timer_width);
}

int ExtentedRangeServo::usToTicks(int usec)
{
    return (int)((double)usec / ((double)REFRESH_USEC / (double)this->timer_width_ticks)*(((double)REFRESH_CPS)/50.0));
}

int ExtentedRangeServo::ticksToUs(int ticks)
{
